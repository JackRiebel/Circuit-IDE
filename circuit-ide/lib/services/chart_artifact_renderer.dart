...
class ChartArtifactRenderer {
  ...
  ChartRenderResult render(ArtifactDocument document) {
    ...
    final svg = _svgFor(document.title, charts, profile);
  ...
  String _svgFor(
    ...
    final height = math.max(560, 278 + (charts.length * chartHeight));
    const width = 1040;
    final buffer = StringBuffer()
      ...
      ..writeln(
        '<metadata>${_xml(jsonEncode({'generator': 'CircuitCode', 'artifact': 'chart_pack', 'chartCount': charts.length, 'pointCount': profile.pointCount, 'highRiskCount': profile.highRiskCount, 'mediumRiskCount': profile.mediumRiskCount, 'lowRiskCount': profile.lowRiskCount, 'hasPoe': profile.hasPoe, 'hasWan': profile.hasWan, 'hasLifecycle': profile.hasLifecycle, 'hasComparison': profile.hasComparison, 'kinds': charts.map((chart) => chart.kind.name).toList(growable: false)}))}</metadata>',
      )
    ...
    for (var chartIndex = 0; chartIndex < charts.length; chartIndex++) {
      _writeChart(
        ...
        charts[chartIndex],
        index: chartIndex + 1,
        top: 246 + (chartIndex * chartHeight),
        width: width,
      );
    }
    buffer.writeln('</g>');
  ...
  void _writeSummary(
    ...
    final signals = [
      ...
      if (profile.hasWan) 'WAN capacity',
      if (profile.hasLifecycle) 'Lifecycle',
      if (profile.hasComparison) 'Model fit',
    ];
    buffer
      ..writeln(
        '<g id="chart-risk-legend" data-has-poe="${profile.hasPoe}" data-has-wan="${profile.hasWan}" data-has-lifecycle="${profile.hasLifecycle}" data-has-comparison="${profile.hasComparison}">',
      )
      ..writeln(
        '<text x="${width - 300}" y="122" fill="#9da6a3" font-family="-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif" font-size="11">Signals: ${_xml(signals.isEmpty ? 'general chart data' : signals.join(' • '))}</text>',
      )
      ..writeln(
