...
class ChartArtifactRenderer {
  ...
  String _svgFor(String title, List<_ChartData> charts) {
    ...
    final buffer = StringBuffer()
      ...
      ..writeln(
        '<metadata>${_xml(jsonEncode({'generator': 'CircuitCode', 'artifact': 'chart_pack', 'chartCount': charts.length, 'pointCount': charts.fold<int>(0, (sum, chart) => sum + chart.points.length), 'kinds': charts.map((chart) => chart.kind.name).toList(growable: false)}))}</metadata>',
      )
