...
class DiagramArtifactRenderer {
  ...
  String _svgFor(
    ...
    final buffer = StringBuffer()
      ...
      ..writeln(
        '<metadata>${_xml(jsonEncode({'generator': 'CircuitCode', 'artifact': 'network_topology_diagram', 'nodeCount': graph.nodes.length, 'edgeCount': graph.edges.length, 'tierCount': tiers.length, 'assumptionCount': assumptions.length, 'siteCount': profile.siteCount, 'mdfCount': profile.mdfCount, 'idfCount': profile.idfCount, 'firewallCount': profile.firewallCount, 'coreSwitchCount': profile.coreSwitchCount, 'accessSwitchCount': profile.accessSwitchCount, 'switchCount': profile.switchCount, 'apCount': profile.apCount, 'accessPortCount': profile.accessPortCount, 'estimatedApPowerWatts': profile.estimatedApPowerWatts, 'apPortLoadPercent': profile.apPortLoadPercent, 'hasDualWan': profile.hasDualWan, 'hasWarmSpare': profile.hasWarmSpare, 'hasPoe': profile.hasPoe, 'hasUpoe': profile.hasUpoe, 'hasMultigig': profile.hasMultigig, 'hasWifi7': profile.hasWifi7, 'readinessItemCount': readinessItems.length, 'capacityItemCount': capacityItems.length, 'linkScheduleCount': linkSchedule.length, 'designZoneCount': tiers.length}))}</metadata>',
      )
