...
class DiagramArtifactRenderer {
  ...
  String _svgFor(
    ...
    final height = math.max(900, 510 + (maxTierSize * 112));
    ...
    final buffer = StringBuffer()
      ...
      ..writeln(
        '<metadata>${_xml(jsonEncode({'generator': 'CircuitCode', 'artifact': 'network_topology_diagram', 'nodeCount': graph.nodes.length, 'edgeCount': graph.edges.length, 'tierCount': tiers.length, 'assumptionCount': assumptions.length, 'siteCount': profile.siteCount, 'mdfCount': profile.mdfCount, 'idfCount': profile.idfCount, 'firewallCount': profile.firewallCount, 'coreSwitchCount': profile.coreSwitchCount, 'accessSwitchCount': profile.accessSwitchCount, 'switchCount': profile.switchCount, 'apCount': profile.apCount, 'hasDualWan': profile.hasDualWan, 'hasWarmSpare': profile.hasWarmSpare, 'hasPoe': profile.hasPoe, 'hasMultigig': profile.hasMultigig, 'hasWifi7': profile.hasWifi7, 'readinessItemCount': readinessItems.length, 'linkScheduleCount': linkSchedule.length, 'designZoneCount': tiers.length}))}</metadata>',
      )
  ...
  void _drawTierBands(
    ...
    for (var i = 0; i < roles.length; i++) {
      ...
      buffer
        ..writeln(
          '<rect class="topology-tier-band" data-tier="${_xml(roles[i].name)}" data-tier-label="${_xml(_tierLabel(roles[i]))}" x="$x" y="206" width="${bandWidth - 12}" height="${height - 486}" rx="18" fill="${i.isEven ? '#151716' : '#181a19'}" stroke="#252928"/>',
        )
        ..writeln(
