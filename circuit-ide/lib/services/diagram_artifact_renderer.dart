...
class DiagramArtifactRenderer {
  ...
  String _svgFor(
    ...
    final buffer = StringBuffer()
      ...
      ..writeln(
        '<metadata>${_xml(jsonEncode({'generator': 'CircuitCode', 'artifact': 'network_topology_diagram', 'nodeCount': graph.nodes.length, 'edgeCount': graph.edges.length, 'tierCount': tiers.length, 'assumptionCount': assumptions.length, 'siteCount': profile.siteCount, 'firewallCount': profile.firewallCount, 'switchCount': profile.switchCount, 'apCount': profile.apCount, 'hasDualWan': profile.hasDualWan, 'hasWarmSpare': profile.hasWarmSpare, 'hasPoe': profile.hasPoe, 'hasMultigig': profile.hasMultigig, 'hasWifi7': profile.hasWifi7}))}</metadata>',
      )
  ...
  void _drawTopologySummary(
    ...
    final cards = [
      ...
      ('Security', '${profile.firewallCount} edge', profile.firewallCount > 0),
      ('Switching', '${profile.switchCount}', profile.switchCount > 0),
